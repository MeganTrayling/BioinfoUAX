...
rep(1,10) #que repita el primer numero el número del segudo
seq(2,6) #secuencia entre los dos números
seq(4,20,by=4) #el número de pasos, es decir en este caso que vaya de 4 en 4
x<-c(2,0,0,4)
y<-c(1,9,9,9)
x+y
x*4
...
y<9
y[4]=1
y<9 #Te dice cuales de tu variable cumplen esa condición booleada (true or false)
y==9 #Te dices cuales son iguales a 9
y!=9 #Cua  les son distintos a 9
y[y<9]=2 #cambiar la condición que todos los elementos menores de 9 sean igual a 2
y[3] <- 4 #cambiar una condición dentro de la variable
y
...
bp
bp["WEIGHT"] #hay que poner comillas
bp[,2] #para pedir una columna y una fila, si ponemos coma 2 es solo para la columna 2 sin filas
bp$WEIGHT #identifica a lo que pones detrás del dolar como una variable dentro de la tabla
bp$WEIGHT[2]
...
genesets<-vector("list",3) #juntamos los 3 genesets, y va a ser un vector con 3 posiciones
genesets[[1]]<-geneset1 #para adjudicar el número de posicioón, PONER DOBLE CORCHETE PARA PODER METERLO EN ESA POSICIÓN, SINO SERÍA SOLO PARA QUE TE DIGA LO QUE HAY AHÍG
genesets[[2]]<-geneset2
genesets[[3]]<-geneset3
genesets

# Ahora le ponemos nombre a los grupos, PARA CAMBIAR EL NOMBRE AL GRUPO
names(genesets)=paste0("set",1:3) #paste 0 para que se ponga en todos los grupos
genesets
...
######################################################################################################################
#############################           IF ELSE IFELSE - para evaluar una condición, hay que seguir dando a ctrl R para que siga el bucle
if(4 > 5) {
  "Verdadero"
} else {
  "Falso"
...
# For acción un numero limitado de veces
dado <- 1:6
for(cara in dado) {
  print(cara ^ 2)
}
...
# While acción hasta que saliese una condición, hasta que se cumpla una condicion
umbral <- 5
valor <- 0

#mientas valor sea menor de umbral, si es menor ok, si no es menor poner que tofavia no y añadir 1 al valor
while(valor < umbral) {
  print("Todavía no.")
  valor <- valor + 1
}
...
head(banco) #primeras columnas y filas de la tabla
dim(banco)#dimensiones
lapply(banco,class)
summary(banco)

# Histograma sacar de una columna/unos datos
hist(x=banco$age)
